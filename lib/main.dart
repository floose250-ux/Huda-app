...
class MainCategoryCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Optimization: Extracted to a const StatelessWidget to allow for framework-level caching
    // and avoid rebuilds when the parent widget (HudaProHome) rebuilds.
    // Impact: Reduces widget build time and CPU cycles during list/grid scrolling.
    return Card(
...
class AzkarCategoriesPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Optimization: Moved static data to a 'static const' member outside the build method.
    // Impact: Prevents re-allocating the list and its map objects on every rebuild,
    // reducing memory pressure and GC cycles.
    return Scaffold(
      backgroundColor: const Color(0xFF000D0D),
