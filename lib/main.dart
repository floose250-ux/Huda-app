  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: GridView.count(
        ...
        crossAxisSpacing: 15,
        children: const [
          MainCategoryCard(title: 'المصحف الشريف', icon: Icons.menu_book, color: Colors.amber, page: null),
          MainCategoryCard(title: 'موسوعة الأذكار', icon: Icons.auto_awesome, color: Colors.orange, page: AzkarCategoriesPage()),
          MainCategoryCard(title: 'جوامع الدعاء', icon: Icons.favorite, color: Colors.redAccent, page: null),
          MainCategoryCard(title: 'الموسوعة الحديثية', icon: Icons.history_edu, color: Colors.blueAccent, page: null),
        ],
      ),
    );
  }
...
/// Standalone const widget for main categories to optimize rebuilds and use canonical instances.
/// This reduces the build time of [HudaProHome] by allowing Flutter to skip rebuilding the
/// category cards when the parent widget rebuilds.
...
class AzkarCategoriesPage extends StatelessWidget {
  ...
  /// Static const data to avoid repeated memory allocations during build.
  /// This optimization prevents the allocation of a new List and several Map objects
  /// every time [AzkarCategoriesPage] is built, improving memory efficiency.
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) => Card(
          ...
          child: ListTile(
            ...
            title: Text(_categories[index]['title'], style: const TextStyle(color: Colors.white, fontSize: 18)),
