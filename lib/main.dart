...
class MainCard extends StatelessWidget {
  ...
  final Color color;
  final Widget? page;
  const MainCard({super.key, required this.title, required this.icon, required this.color, this.page});
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: InkWell(
        ...
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, size: 45, color: color), const SizedBox(height: 10), Text(title, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold))]),
      ),
    );
  }
}
...
class AzkarCategoriesPage extends StatelessWidget {
  ...
  // Moved categories to a static const list to avoid redundant allocations on every rebuild.
